...
## 2026-06-23 — 智能动态提醒 + 主进程防崩 + 提醒交互打磨

### 完成内容
- **提醒改"活动感知"**（`reminders.js` 重写）：弃用固定间隔死闹钟，改用
  `powerMonitor.getSystemIdleTime()` 按"在电脑前连续坐了多久"提醒。离开 ≥ `BREAK_SEC`
  （默认 5 分钟）当作已休息 → 计时清零；只有人在座（idle < `PRESENT_SEC`=60s）才弹；
  每 5 秒轮询。久坐间隔默认 50→**60 分钟**（`settings.js` + 持久化配置同步）。
- **主进程防崩**（`wander.js`）：`setPosition` 偶发收到 NaN 坐标会让 Electron 抛
  "conversion failure" 崩溃。新增 `setPos`/`readPos` 边界校验，非法坐标跳过该帧 + 留日志。
  `behavior.js returnHome` 同步加坐标兜底，避免 dur=NaN 导致缓动卡死。
- **提醒交互**：提醒进行中点猫 = 收气泡 + 清队列 + 进入 DRAG，松手缓动回角落（不打扰）；
  "回角落"由固定 600ms 改为**按距离匀速**（`RETURN_SPEED`），远距离不再像瞬移飞回；
  久坐提醒漫游加宽（轮数 2→3、每段 2.6s→5.5s）；气泡文案 5→16 条；提醒队列同类去重。

### 关键决策
- 点猫结束提醒走成熟的 DRAG 路径而非"按下即回家"：后者会与拖拽 `dragMove` 抢写窗口位置导致抖动。
- `PRESENT_SEC`(60s) 与 `BREAK_SEC`(300s) 之间为"挂起窗口"：人短暂离开不重置、回来补发；
  <5 分钟的短暂停顿算作连续在座（不视为真休息）。

### 遗留问题 / 下次继续
- 测试用环境变量：`PET_STANDUP_MS`（坐够多久，ms）/ `PET_BREAK_SEC`（离开多久算休息）可压短自测。
- 活动感知逻辑仅在 Mac 验证；Windows 真机待测。

## 2026-06-23 — 待机动作分档 + 安静默认姿势 + 切换淡入淡出

### 完成内容
- **动作分三档**（解决"拍照一直循环很假"）：新增 `idle.holdSec` map（动作名→播放秒数）。
  在 map 里的=过场类，播完退回长驻姿势；不在的=长驻类，循环到下次 switch。
  - 极短：拍照 3s；偶尔：趴着 10s / 睡觉·电脑 12s；长驻：摆手·趴着前的默认·困。
- **安静默认姿势**：用 PIL 逐帧量化各动作"动静值"，趴着(40)最闹、困(7.2)最静。
  把待机默认/兜底姿势从 `pawo` 改为 `kun`（`SETTLE_POSE='kun'`），趴着降为偶尔档。
- **删重复素材**：`naotou`(实为第二个电脑桌) / `tiaoyue`(实为躺地打滚) 文件名与画面不符且
  与电脑/趴着撞脸，已从动作池移除（素材文件暂留磁盘，未删）。
- **切换淡入淡出**：渲染层 `onSetAction` 改为先淡出(opacity→0)→换图+mask→淡入，
  `#cat` 加 `transition: opacity 0.15s`，消除硬切突兀。
- 新增纯函数 `isTransient` / `pickPose`（actions.js）；behavior 用 `gestureTimer` 管过场退回。

### 关键决策
- 用"逐帧像素变化量"客观选最安静动作，而非凭感觉——证实用户直觉（趴着最闹）。
- 分档用 holdSec map 而非硬编码动作类型：加/改动作时长只改 settings，不动代码（延续项目"配置驱动"风格）。
- 独立 code-reviewer 复审抓出 2 个 MEDIUM 已修：① 过场退回时重新取时段(防跨时段边界)；
  ② 淡出期间先清 mask(防按旧剪影误判命中)。

### 遗留问题 / 下次继续
- `naotou` / `tiaoyue` 素材文件仍在磁盘（assets/actions/），确认无用后可删目录。
- 偶尔档(睡觉/电脑)与长驻档(摆手/困)只在夜间/黄昏时段可见，白天未肉眼验收，已靠逻辑测试覆盖。
